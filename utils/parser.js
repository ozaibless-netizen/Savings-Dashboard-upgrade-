...
/**
 * Parse SMS message to extract financial transaction details
 * Supports: Airtel, TNM, Vodacom, Mpamba, betting, airtime
 */
...
export function analyzeTransactions(transactions) {
  ...
  for (const item of transactions) {
    if (seen.has(item.tid || item.id)) continue;
    seen.add(item.tid || item.id);

    count++;
    biggest = Math.max(biggest, item.amount || 0);

    if (item.type === 'income') {
      income += item.amount || 0;

      if (item.savingsStatus === 'approved') {
        approvedSavings += item.saveAmount || 0;
      }

      if (item.savingsStatus === 'pending') {
        pendingSavings += item.saveAmount || 0;
      }
    }

    if (['expense', 'gambling', 'airtime'].includes(item.type)) {
...
export function predict(balance, income, expense) {
  const trend = income - expense;
  const projected = balance + (trend * 7);

  let risk = 'LOW RISK';
  if (projected < balance * 0.5) {
    risk = 'HIGH RISK';
  } else if (projected < balance) {
    risk = 'MEDIUM RISK';
  }

  return { projected, risk, trend };
}
